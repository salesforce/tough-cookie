...
describe('Node util module fallback for non-node environments', () => {
  describe('getCustomInspectSymbol', () => {
    it('should not be null in a node environment', () => {
      expect(getCustomInspectSymbol()).toEqual(Symbol.for("nodejs.util.inspect.custom") || util.inspect.custom)
    })

    it('should not be null in a node environment when custom inspect symbol cannot be retrieved (< node v10.12.0', () => {
      expect(getCustomInspectSymbol({
  ...
  describe('util usage in MemoryCookie', () => {
    ...
    describe('when store is empty', () => {
      ...
      it("fallback produces equivalent output to custom inspect", () => {
        expect(util.inspect(memoryStore.idx)).toEqual(inspectFallback(memoryStore.idx))
      })
    })
    ...
    describe('when store has multiple cookies', () => {
      beforeEach(async () => {
        ...
        await cookieJar.setCookie('d=3; Domain=example.com; Path=/some-path/', url)
        await cookieJar.setCookie('e=4; Domain=example.com; Path=/some-path/', url)
        await cookieJar.setCookie('f=5; Domain=another.com; Path=/', "http://another.com/index.html")
      })
