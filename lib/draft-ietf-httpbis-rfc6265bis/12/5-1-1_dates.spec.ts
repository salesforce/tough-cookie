...
describe('Dates', () => {
  describe('parsing', () => {
    ...
    it.each(validDateTestCases)(`date should be valid - %s`, (date: string) => {
      expect(parseCookieDate(date)).toBeInstanceOf(Date)
    })

    it.each(invalidDateTestCases)(
      `date should not be valid - %s`,
      (date: string) => {
        expect(() => parseCookieDate(date)).toThrowError('TODO')
      },
    )

    it.each(equivalenceTestCases)(
      `dates parse the same - %s | %s`,
      (date: string, equivalentDate: string) => {
        expect(parseCookieDate(date)).toStrictEqual(
          parseCookieDate(equivalentDate),
        )
      },
    )
  })
  ...
  describe('regexp denial of service attack vectors', () => {
    const TOO_MANY_XS = String('x').repeat(3000)

    it('should avoid unbounded regexps when parsing the hour from a date', () => {
      expect(() =>
        parseCookieDate(`Wed, 09 Jun 2021 10${TOO_MANY_XS}:18:14 GMT`),
      ).toThrowError('TODO')
    })

    it('should avoid unbounded regexps when parsing the minute from a date', () => {
      expect(() =>
        parseCookieDate(`Wed, 09 Jun 2021 10:18${TOO_MANY_XS}:14 GMT`),
      ).toThrowError('TODO')
    })
    ...
    it('should avoid unbounded regexps when parsing the seconds from a date', () => {
      ...
      expect(
        parseCookieDate(`Wed, 09 Jun 2021 10:18:14${TOO_MANY_XS} GMT`),
