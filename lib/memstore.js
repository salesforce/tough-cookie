...
MemoryCookieStore.prototype.removeCookies = function removeCookies(spec, cb) {
  if (!this.idx[spec.domain]) {
    if (spec.path) {
      delete this.idx[spec.domain][spec.path];
    } else {
      delete this.idx[spec.domain];
    }
  }
  return cb(null);
};
